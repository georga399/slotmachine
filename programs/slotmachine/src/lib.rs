...
declare_id!("HpKZfyCptLBsFKtxFQiyWHNCpBmpWKERvZXY8YrSnLD4");
...
pub mod slotmachine {
    ...
    pub fn withdraw_from_vault(ctx: Context<WithdrawFromVault>, amount: u64) -> Result<()> {
        ...
        // Проверяем, что есть достаточно средств (оставляем rent-exempt)
        let available = vault_lamports.checked_sub(RENT).ok_or(SlotmachineError::InsufficientFunds)?;
        
        require!(
            amount <= available,
            SlotmachineError::InsufficientFunds
        );

        // Переводим средства из vault в authority
        **ctx
            .accounts
            .vault
            .to_account_info()
        ...
        **ctx
            ...
            .authority
            .to_account_info()
            .try_borrow_mut_lamports()? = authority_lamports.checked_add(amount).unwrap();

        msg!("Withdrawn {} lamports from vault to authority", amount);
